...
public class MyDeadboltHandler extends AbstractDeadboltHandler {
	...
	public Promise<Optional<Result>> beforeAuthCheck(final Http.Context context) {
		if (PlayAuthenticate.isLoggedIn(context.session())) {
            ...
            return Promise.promise(new F.Function0<Optional<Result>>()
            {
                @Override
	...
	public Promise<Optional<Subject>> getSubject(final Http.Context context) {
		...
		return Promise.pure(Optional.ofNullable((Subject)User.findByAuthUserIdentity(u)));
	}
	...
	@Override
	public Promise<Result> onAuthFailure(final Http.Context context,
